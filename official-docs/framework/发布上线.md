...
通过 We 分析 AB 实验进行灰度。
Skyline
默认是需要经过 We 分析的 AB 实验的
，也就是小程序新版本发布后，默认还是以 WebView 运行，需要在 We 分析的 AB 实验的「小程序基础库实验」逐步放量。需要特别留意的是，
当 AB 实验的流量分配到 100% 时，并不代表是全量，而是 Skyline 和 WebView 各 50%
，若要全量的话，需要先结束实验再选择全量某一个实验组。
通过小程序版本管理中的发布灰度。
