...
支付刷脸设备也是通过
initByCaller
接口发起通话，但是在参数上有一些区别。支付刷脸设备请使用下列 businessType：
businessType
业务类型
caller.id
listener.id
...
#
其他联系人绑定
其他联系人由管理员分享页面进行绑定。即管理员在小程序中点击「分享」按钮, 并通过
`wmpfVoip.getBindContactPath`
获取插件分享页面链接，并在页面的
`onShareAppMessage`
中设置。
绑定完成后，会跳转到
