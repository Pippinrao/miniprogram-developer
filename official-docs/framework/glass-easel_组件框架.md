...
后端
是 glass-easel 的一个重要概念，表示组件系统的运行环境。在 web 环境下运行时，后端是浏览器的 DOM 接口；在小程序环境下运行时，后端则是小程序环境接口。这使得（后端无关的）组件代码可以运行在不同环境下。
...
glass-easel 组件框架在
GitHub
上开源，代码和更详细的文档、示例等可以在 GitHub 上找到。
#
适配指引
glass-easel 适配指引
中列举了一些相较于现有组件框架
exparser
需要变更的逻辑，可以用于将现有的小程序迁移到新的框架，也可以用于快速了解新旧框架之间的差异。