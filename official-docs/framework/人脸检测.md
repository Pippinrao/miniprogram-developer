...
#
人脸关键点检测
VisionKit 从基础库 2.25.0 版本 (安卓微信>=8.0.25，iOS微信>=8.0.24) 开始提供人脸关键点检测，作为与
`marker 能力`
和
`OSD 能力`
平行的能力接口。
...
1. 静态图片检测
通过
VKSession.detectFace 接口
输入一张图像，算法检测到图像中的人脸，然后通过
VKSession.on 接口
输出人脸位置坐标、106个关键点坐标以及人脸在三维坐标系中的旋转角度。
示例代码：
...
算法实时检测相机中的人脸，通过
VKSession.on 接口
实时输出人脸位置坐标、106个关键点坐标以及人脸在三维坐标系中的旋转角度。
示例代码：
...
#
3. 开启3D关键点检测
想要开启人脸3D关键点检测能力，静态图片模式仅需要在2D调用基础上增加
`open3d`
字段，如下
...
#
特别说明
若小程序人脸识别功能涉及采集、存储用户生物特征（如人脸照片或视频、身份证和手持身份证、身份证照和免冠照等），此类型服务需使用
微信原生人脸识别接口