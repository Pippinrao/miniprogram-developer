...
通过
服务端接口
可以获取打开小程序任意页面的 URL Link。适用于从短信、邮件、网页、微信内等场景打开小程序。
...
#
拼接参数
将原有 URL Link 平滑升级为加密 URL Link，支持开发者自行在链接后面拼接参数
`CUSTOM PARAMETE`
,拼接参数后的 URL Link 如下所示：
...
是一种特殊的
`query`
，最大256个字符，只支持数字，大小写英文以及部分特殊字符：!#$&'()*+,/:;=?@-._~%`，需要url_encode;
在本次规则调整生效前已经生成的 URL Scheme 可继续正常使用，并可直接进行
`CUSTOM PARAMETE`
参数拼接；
