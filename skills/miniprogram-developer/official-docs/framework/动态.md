...
#
1.4.19 (2026-05-06)
修复 word-break: all 文本截断问题
修复 video 周围黑线问题
修复 wxss 动画贝塞尔曲线表现错误
修复 conic-gradient 渐变问题
修复 scroll-view 下拉刷新影响无障碍
修复 scroll-view 自动撑高问题
修复 image 在退页后仍回调 onerror 事件的问题
修复 wxss animation 无效输入下的闪退问题
修复 input composition 状态在 setData 后丢失的问题
#
1.4.18 (2026-04-16)
更新 鸿蒙 skyline 内存占用优化
更新 开发者工具 electron 版本支持 skyline
更新 新增样式表动态更新接口以支持开发者工具热更新
更新 scroll-view anchoring 功能增强
更新 setData callback 使用 skyline 渲染回调
修复 scroll-view 并行布局问题
#
1.4.17 (2026-03-23)
优化 引擎内存占用，提升整体性能
优化 大图加载时的内存消耗
修复 文本布局丢失或错位的问题
修复 swiper 组件在 circular 模式下开启 cache-extent 后渲染错误的问题
修复 IntersectionObserver 在特定条件下不触发回调的问题
修复 scroll-view 的 scroll-top / scroll-left 属性，保证在之前提交的节点操作完成后才进行滚动
#
1.4.16 (2026-02-09)
新增 CSS 支持 currentColor
新增 text 组件支持自定义选区弹出菜单
新增 iOS 端支持系统“粗体文本”配置
修复 自定义路由卡死问题
修复 IntersectionObserver 未设置 thresholds 无法触发回调的问题
修复 map custom callout id 溢出问题
修复 open-container 返回页面无法快速点击的问题
修复 文本意外换行问题
修复 scroll-view 滑动首次加载跳动的问题
修复 input 光标选区颜色显示错误的问题
修复 文本 text overflow 无法去除省略号的问题
#
1.4.15 (2026-01-09)
新增 image 组件的 preload 属性，用于图片预加载
优化 当页面被遮挡时自动停止动画，减少资源消耗
修复 iOS 上某个导致崩溃的问题
修复 鸿蒙平台无障碍功能导致的闪退
修复 fixed 定位元素移动后未重新计算层级的问题
修复 box-shadow 在 CSS 动画中不生效及导致的闪退问题
修复 gif/apng 动图帧率错误的问题
修复 swiper 组件在某些情况下未触发 change 事件的问题
修复 在 font 和 animation 简写属性中无法使用 CSS 变量的问题
修复 安卓平台无障碍功能导致的闪退
修复 图片预加载配置项不生效的问题
修复 scroll-view 组件在首次加载时出现跳动的问题
#
1.4.14 (2025-12-18)
新增 text 组件支持行内显示
新增 image 组件支持 loadstart 事件
优化 scroll-view 的滚动锚定行为，提升内容变化时的稳定性
优化 HTTP 客户端，移除并发限制以提升网络请求性能
修复 image 组件在特定模式下图片不显示的问题
修复 组件事件回调中潜在的崩溃问题
修复 iOS 上 JavaScript 回调导致的崩溃
修复 文本节点更新时可能引发的死锁问题
修复 键盘高度变化事件输出错误值的问题
修复 IntersectionObserver 在未设置 thresholds 时无法触发回调的问题
修复 移除子组件操作时可能发生的崩溃
修复 布局节点访问父节点时可能导致的崩溃
修复 无障碍功能在鸿蒙系统上的崩溃问题
修复 swiper 组件应始终触发 change 事件
修复 scroll-view 滑动时首次加载内容跳动的问题
#
1.4.13 (2025-11-25)
新增 支持 CSS :host 选择器
优化 scroll-view 滚动性能，提升渲染效率
修复 gap 属性与 CSS 变量结合时失效的问题
修复 flex 布局中使用 gap 导致元素展示不全的问题
修复 snapshot 组件 pointer-events 属性不生效的问题
修复 图片在尺寸为零时渲染错误的问题
修复 em 单位计算中基准 fontSize 错误的问题
修复 Intersection Observer 在某些情况下崩溃的问题
修复 svg 图片加载时可能出现的死循环问题
修复 使用 mask-image 展示 svg 图片时出现灰色边框的问题
修复 open-container 组件纵向测量不准确的问题
修复 图片闪烁问题
修复 动图设置目标尺寸后显示异常的问题
修复 伪元素节点连接问题导致交互失效
#
1.4.12 (2025-10-31)
新增 layout paragraph 支持无障碍功能
优化 文本空白字符处理流程
修复 grid-view 布局异常
修复 IntersectionObserver attached 时机失效
修复 scroll-view 下拉刷新动画异常
修复 iOS 平台原生视图异常消失
修复 横向手势返回操作异常
修复 IntersectionObserver target 节点异常时的崩溃
#
1.4.11 (2025-09-15)
修复 文本末行换行符溢出仍出省略号
修复 input 取消 composite 后草稿字符丢失
修复 scrol-view scroll-anchoring 偶现意外跳动
修复 swiper 开启循环显示后 animateTo 动画错误
修复 scroll-view 自动撑高问题
修复 自定义路由 barrierDismissible 两次返回
修复 伪元素节点消失仍在播放 css animation
修复 文本中 span 意外换行
修复 scroll-view 不足一屏时不触发 lower / upper 事件
修复 input / textarea maxlength 输入 emoji 闪退
#
1.4.10 (2025-08-28)
新增 text 增加 trailing-spaces 属性支持多行文本末行末尾预留空间
新增 IntersectionObserver 支持多次监听
新增 open-container 支持通过接口方式触发打开
优化 css animation 在节点不可见时停止动画
优化 scroll-view scroll-anchoring 支持度优化
优化 open-container 手势返回时支持上下拖动页面
修复 swiper 开启自动播放后，隐藏 & 显示会失效
修复 image gif 动画修改 src 后动画速度异常
修复 word-break: break-all 需要断开数字、英文、符号
更新 grid-view 增加子节点后白屏
修复 swiper 更新高度后动画失效
修复 scroll-view 嵌套 swiper 时可能导致切换无动画
修复 图片渲染变形
修复 open-container 手势返回动画消失
修复 键盘上推后无法恢复
#
1.4.9 (2025-08-06)
新增 large-image 支持大图渲染
优化 css animation 无限循环动画自动开启 repaint boundary 避免大面积重绘
优化 text 绘制性能
优化 image 对 svg 格式的判断
优化 渲染树结构优化
修复 span 丢失问题
修复 sticky-header 动态增加内容崩溃
修复 gif 动画消失
修复 swiper animation 被打断时 bind:change 和 bind:animationfinished 没有回调
修复 grid-view 删除并交换元素后布局错位
修复 css 文本 baseline shortcut 问题
修复 line-height 无法更新回 normal 值
修复 swiper current 更新问题
修复 picker-view indicator-style 闪退
修复 iOS input 失焦的同时无法 focus
修复 某些白屏及 crash 问题
#
1.4.8 (2025-05-27)
优化 文本测量优化
修复 listView 直接子节点带 margin 时 getBoundingClientRect 获取错误
修复 input 失焦后无 blur 事件
修复 intersection observer 添加对 list-view 处理
修复 gif 动画频率错误
修复 list-builder 偶现无法回滚
修复 input selection style 不符合预期
#
1.4.7 (2025-04-29)
新增 text 组件支持 space / decode 属性
新增  HarmonyOS 下支持暗黑模式
修复 textarea 在键盘收起来时都没有给 onKeyboardHeightChange 事件
修复 attached 时机创建 intersecton observer root 失效
修复 intersection observer target 离开视窗后 root-margin 未生效
修复 swiper next-margin 空出的区域无法点击
修复 swiper 嵌套 grid-view 无法恢复滚动位置
修复 切换底部 tab 页面闪退
修复 设置 word-break 无法点击
修复 text max-lines 字符串解析失败会导致闪退
修复 input / text keyboardHeight blur 时不触发
修复 图片解码死循环
修复 touch 偶现 crash
#
1.4.6 (2025-04-08)
...
优化 自定义字体隔离
优化 开发者工具内核升级
修复 两个 sticky-section 滚动速度不一致
修复 调整文字选区的默认背景色
修复 若干 IntersectionListener 相关接口表现异常及 crash
修复 picker-view 设置非法值出现滚动
修复 swiper animationfinish 返回 current 参数不正确
修复 input 字体样式错误
修复 picker-view-column 无子项时崩溃
修复 循环动画时无法触发 scroll-into-view
修复 swiper 更新高度后动画失效
#
1.4.5 (2025-03-19)
新增 css 支持 word-break: break-all
新增 IntersectionObserver 支持同一个节点多个监听、指定 margins、relativeTo 特定 scroll-view 节点
新增 @keyframes 样式可全局共享（由 rendererOptions.skyline.keyframeStyleIsolation 控制）
优化 swiper 组件切换动画曲线，改进与 scroll-view 组合使用的体验
优化 卡顿率计算
优化 worklet 键盘跟随动画不贴合键盘
修复 image 内存泄漏
修复 input placeholder-style 使用 var 会导致 crash
修复 swiper 设置 current 失效
修复 swiper 开启 circular 之后首次可能会访问到最后一个元素，并且 circular 特性失效
修复 swiper current / bindchange 失效
修复 有 overflow hidden 时可能导致 IntersectionObserver 失效
修复 键盘上推未恢复到原位
修复 节点的 inline style 更新时会覆盖通过 applyAnimatedStyle 绑定的动画
修复 安卓原生组件存在内存泄漏
修复 sticky-header 仅第一个元素支持 offset-top 且不支持负值
修复 css transition box-shadow 闪退
修复 image 组件切换 src 后，长按识别二维码不生效，且 bind:load 未触发
修复 css variable 处理未定义的变量
修复 ios input focus 失败
修复 iOS 图片可能存在模糊
修复 scroll-view 组件的 scroll-into-view 在某些情况下失效
#
1.4.4 (2024-12-23)
新增 rich-text 支持 mode=web-static 模式，优化渲染性能（基础库 3.7.7）
优化 伪元素文本布局优化
修复 scroll-view 组件 scroll-anchoring 引发额外动画
修复 scroll-view 组件 nested scroll 下 bind:refresherpulling 不断触发
修复 文本在某些情况下未能正确换行
修复 在手势操作过程中改变 opacity 会使手势会被打断
修复 IntersectionObserver 重新绑定无法触发回调
修复 文本点击引起的 tap 失效问题
修复 image 组件支持 padding / border
修复 地图在手势过程中，可能出现只能放大无法拖动的情况
修复 键盘上推异常
修复 若干 crash
#
1.4.3 (2024-12-02)
新增 WXML 真机调试选择节点时支持 overlay 展示
新增 swiper 组件 previous-margin 和 next-margin 添加 rpx 支持（基础库 3.7.6）
新增 scroll-view 返回触顶速度及设置初速度（scroll 事件返回 boundaryVelocity 字段及
worklet.scrollViewContext.scrollTo
）
优化 样式匹配流程性能优化
优化 样式应用流程性能优化
优化 文本布局性能优化
修复 picker-view 组件 bind:change / bind:pickstart / bind:pickend 多次触发
修复 swiper 组件 display-multiple-items 滑动元素问题
修复 scroll-view 组件去掉 overscroll indicator
修复 css mask none 导致背景图无法绘制
修复 scroll-view 组件 nested scroll 滚动衔接问题
修复 css background gradient 渲染异常问题
修复 离屏删除节点存在内存泄漏
修复 webview 页面跳 skyline 页面再返回存在内存泄漏
修复 text 节点设置 pointer events 仍可点击
修复 若干 crash
#
1.4.2 (2024-11-19)
...
#
1.3.4 (2024-07-11)
新增 tag 选择器增加 legacy 选项以兼容 WebView（不受组件隔离限制）
详情
...
新增 scroll-view 组件 nested 模式的 nested-scroll-body 支持 offset-top 属性
详情
...
1.2.0（2024-01-08）
新增 开发者工具支持真机调试
新增 CSS 支持 flex order
详情
新增 CSS 支持 will-change: contents，用于声明绘制边界
详情
新增 支持全局跨页面组件
详情
新增 支持 apng 动图
新增 scroll-view 组件支持 builder 模式，用于按需构建在屏节点
详情
新增 picker-view 组件支持 indicator-style 属性
新增 input 键盘动画提供 worklet 回调，用于实现键盘跟随动画
详情
新增 textare 组件支持 linechange 事件
新增 worklet 增加 ref 机制，用于在 ui 线程操作节点
详情
新增 worklet 支持 scrollTo 接口
详情
优化 渲染性能
...
新增
swiper
组件支持一批新的交互动画类型，用于实现常见的原生交互效果
新增 scroll-view 组件支持
type="nested"
，用于处理 scroll-view 嵌套的场景
新增 input 组件支持
cursor-color
属性，可自定义光标颜色
新增 input 组件支持
compositionstart
/ compositionupdate / compositionend 事件，用于感知输入草稿状态
新增 input 组件支持
selectionchange
事件，用于感知光标位置变化
...
新增
snapshot
截图组件，用于将组件内的 WXML 内容导出图片
新增
sticky-header
组件支持 offset-top 属性，用于设置顶部偏移
新增
text
组件支持 overflow 属性，用于对齐文本省略特性
新增 scroll-view 组件下的直接子节点 height 支持 % 单位
新增 开发者工具上支持 backgroundColorContent 配置项
详情
...
新增 CSS 支持 calc 函数
详情
新增 CSS 支持伪元素 before 和 after
详情
新增 CSS 支持 var 函数
详情
新增 CSS 支持 mask-image 属性
详情
新增 支持
picker-view
组件
...
/
sticky-section
组件支持 padding 属性，设置组件内部的内边距
新增
scroll-view
组件直接子节点支持 CSS margin
新增
scroll-view
组件支持 min-drag-distance 属性，设置起始滚动阈值
新增 text / span 组件支持内联 view 等普通节点
新增 支持新版本组件框架 glass-easel
详情
优化 完善 SVG 支持度
优化 自定义路由接口设计，以及启动页无法绑定自定义路由
详情
...
修复
swiper
组件 circular & autoplay 动态切换时表现异常
修复
swiper
组件循环模式下 cache-extent 异常
...
修复
scroll-view
组件下使用 image 组件的 fade-in 属性时布局异常的问题
...
修复
scroll-view
组件 bindscrolltolower 和 bindscrolltoupper 多次触发问题
...
#
0.9.15（2023-01-16）
新增 支持 grid-view 以实现网络布局、瀑布流布局等
详情
新增 支持 share-element 更多自定义特性
详情
新增 完善 swiper 特性支持（bindtransition、bindanimationfinish、display-multiple-items）
详情
新增 swiper 组件支持手势协商
详情
新增 swiper 支持 scroll-with-animation 及 cache-extent 属性
详情
新增 支持解析 CSS text-shadow
新增 与 applyAnimatedStyle 配套使用的 clearAnimatedStyle 接口
新增 doubletap 手势返回坐标信息
详情
优化 统一 skyline 的释放流程
优化 font-face 增加本地缓存
优化 image 组件重新渲染相关逻辑
...
详情
新增 支持 rem 单位
新增 scroll-view 支持 custom type
详情
新增 scroll-view 下拉刷新增加 willRefresh 事件和 bounce 优化
详情
新增 scroll-view 支持 scrollend 事件和 scrolling reason
详情
新增 scroll-view 增加 scroll-into-view-alignment / scroll-into-view-within-extent 属性
详情
新增 scroll-view 增加 cache-extent 属性
详情
新增 小程序页面背景颜色支持
优化 图片缓存内存占用
优化 skyline 释放流程
...
详情
新增 scroll-view reverse 属性，支持反向滚动
详情
新增 CSS 支持 first-child / last-child 伪类
