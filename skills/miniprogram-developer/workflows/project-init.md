...
| 场景 | 处理 |
|------|------|
| 目标目录已存在 | 询问用户：覆盖/合并/选择新目录/取消 |
| npm install 失败 | 检查 Node.js 版本（需 ≥16），检查网络连接，提示用户检查 npm 镜像源 |
| AppID 无效或未提供 | 使用测试号（touristappid）继续创建，提示用户后续在 project.config.json 中修改 |
| 微信开发者工具无法打开项目 | 检查工具安装路径，提示用户手动打开并导入项目 |
| project-scaffold 子Agent返回 error | 展示错误详情，请用户确认配置后重试 |
| 组件库安装失败（npm包不存在） | 降级为微信原生组件，标注"组件库未安装，使用基础组件" |
| TypeScript 模板生成失败 | 降级为 JavaScript 模板，提示用户可后续手动迁移 |
